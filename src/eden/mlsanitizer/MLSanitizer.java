...
public class MLSanitizer {
  ...
  public static void main(String[] args) {
    System.exit(new MLSanitizer(args).run());
  }
  ...
  private int run() {
    ...
    for (String argument : this.arguments) {
      ...
      try {
        ...
        read();
        if (getContext().isClean())
          continue;
        in = new BufferedInputStream(Files.newInputStream(file.toPath()));
        index = argument.lastIndexOf(".");
        getContext().setForWriting(
            in,
            new BufferedOutputStream(Files.newOutputStream(Paths.get(
                argument.substring(0, index) + SUFFIX
  ...
  private void read() throws BadPDFException, IOException {
    main:
    while (getContext().readLine() != null)
    ...
    if (getContext().hasOpenObjs())
      throw new PDFObjectOpenException(EDENExceptions.makeSubject(
          this.item, Long.toString(getContext().getObjMark())));
  }
  ...
  private void readInfo() {
    if (readInfo(Flag.AUTHOR, REGEX_AUTHOR)
  ...
  private void readPage() {
    if (getContext().lineMatches(REGEX_ANNOTS))
      getContext().addLine();
    else
      readHook();
  }
  ...
  private void readPdf() throws NonPDFException {
    if (getContext().getLineCount() > 1)
      throw new NonPDFException(this.item);
    seek(REGEX_PDF, Mode.PAGES);
  }

  private void readUri() {
    if (getContext().lineMatches(REGEX_URI))
      getContext().setMode(getContext().getDistanceFromMark() == DIST_HOOK_URI
          ? Mode.CATALOG : Mode.HOOK);
  }

  private void seek(Pattern pattern, Mode mode) {
    if (getContext().lineMatches(pattern))
  ...
  private void writePage() throws IOException {
    if (getContext().lineMatches(REGEX_CONTENTS)) {
      ...
      for (int index = 0; index < contents.length - 4; index++)
        builder.append(contents[index]).append(SPACE);
      getContext().writeString(
  ...
  private void writeXref() throws IOException {
    if (getContext().isNotInObj())
      if (getContext().isMarkSet()) {
        ...
        if (index > -1)
          getContext().setMark(getContext().getSize() + index);
      }
    getContext().writeBuffer();
  }
